...
long long			floating_value(int accurancy, long double data)
{
	long long	c;
	long long	k;
...
int					print_float(t_mask *mask, long double data, int count)
{
	...
	minus = (data < 0) ? 1 : 0;
	(minus) ? data *= -1 : 0;
	float_data = ft_itoa(floating_value(acc, data));
	int_data = (*float_data == '2') ? ft_itoa((long)data + 1) : ft_itoa(data);
	nspaces = (mask->width > acc + ft_strlen(int_data)) ? mask->width - acc - ft_strlen(int_data) : 0;
	...
	(mask->accurancy != 0) ? count += write(1, float_data + 1, acc) : 0;
